        ...
        describe('Tasklists are active', () => {
            ...
            describe('Key 46 - delete', () => {
                ...
                beforeEach(async () => {
                    ...
                    dispatch = MakeTestReducer(initState);
                    GapiTasks = {
                        deleteTasklist: jest.fn(() => Promise.resolve({}))
                    };
                    keydownListener = MakeKeyDownListener(initState, dispatch, GapiTasks);
                });
                ...
                it('removed task both locally and via api', async () => {
                    ...
                    expect(GapiTasks.deleteTasklist.mock.calls.length).toBe(1);
                    expect(GapiTasks.deleteTasklist.mock.calls[0][0]).toBe(initState.items[initState.cursor].id);
                });
            });

            describe('Key 13 - enter', () => {

                it('inserts empty value before creation', async () => {
                    ...
                    expect(newState.isNextBlurInsertion).toBe(true);
                    expect(newState.items).toStrictEqual([{ title: ''}, ...initState.items]);
                });

                it('edits a tasklist', async () => {
